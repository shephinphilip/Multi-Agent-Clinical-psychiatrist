...
# Optimized for 1M concurrent API calls without Redis or Prometheus
# Key Optimizations:
# 1. FULL ASYNC: Motor for MongoDB, AsyncOpenAI for LLMs, async HF pipelines.
# 2. CONCURRENCY: Semaphore limits LLM calls (100/worker), async workers.
# 3. LIGHTWEIGHT MODELS: gpt-4o-mini for all LLM tasks to reduce latency.
# 4. STATELESS: No session state stored locally; MongoDB for persistence.
# 5. DB OPTIMIZATION: Indexed MongoDB queries, batch operations.
# 6. LOAD SHEDDING: Fallback to generic responses if LLM calls exceed 500ms.
# 7. DEPLOYMENT: Gunicorn + Uvicorn (100 workers), Kubernetes for scaling.
# ASSUMPTIONS: Deploy with 100 Gunicorn workers, MongoDB replica set, Kubernetes.
# Install: pip install fastapi uvicorn motor langchain langchain-openai transformers aiofiles
import asyncio
from motor.motor_asyncio import AsyncIOMotorClient
from motor.motor_asyncio import AsyncIOMotorCollection
import redis
import os, re, json, torch, datetime, logging, numpy as np
from typing import Annotated, Optional, List, Dict, Any, cast, TypedDict, TYPE_CHECKING, Callable, AsyncGenerator
...
from redis.asyncio import Redis  # Explicitly import the asyncio Redis client
...
class ResponseCache:
    ...
    def _make_key(self, user_text: str, category: str, sentiment: str) -> str:
        """Create cache key from inputs"""
        combined = f"{user_text.lower()[:100]}|{category}|{sentiment}"
        return hashlib.md5(combined.encode()).hexdigest()
    
    def get(self, user_text: str, category: str, sentiment: str) -> Optional[str]:
        """Retrieve cached response if valid"""
        key = self._make_key(user_text, category, sentiment)
    ...
    def set(self, user_text: str, category: str, sentiment: str, response: str) -> None:
        """Store response in cache"""
        key = self._make_key(user_text, category, sentiment)
        self.cache[key] = (response, time_module.time())
        logger.debug(f"Cached response for: {user_text[:50]} (cache_size={len(self.cache)})")
...
response_cache = ResponseCache(ttl_seconds=600)  # 10-minute TTL

# ============================================================
#  FASTAPI LIFESPAN CONTEXT (must be defined BEFORE app instantiation)
# ============================================================
@asynccontextmanager
async def lifespan(app: FastAPI):
    """
    Startup: Initialize all components (models, embeddings, MongoDB).
    ...
    # NEW: Start background cache cleanup task
    ...
    if client is not None:
        client.close()  # type: ignore # Motor close is sync
...
app = FastAPI(
    title="Zenark Mental Health API",
    version="2.0",
    description="Empathetic AI counseling system with detailed logging.",
    lifespan=lifespan,  # type: ignore # Pass lifespan context here
)
app.add_middleware(
    CORSMiddleware,
...
async_embedding_function = None  # ASYNC EMBEDDINGS
...
# NEW: Global semaphore for LLM calls (limit to 100 concurrent to prevent OOM/throttling)
LLM_SEMAPHORE = asyncio.Semaphore(100)

# NEW: Redis client for caching
# Global redis_client with type hint
...
# ============================================================
# GLOBAL MODEL CLIENTS (ASYNC & REUSED)
# ============================================================
# Async LLM clients
...
async def initialize_models() -> None:
    global initialized, emotion_classifier, sentiment_classifier
    if initialized:
        return
    try:
        if HF_TOKEN:
...
# Async pipeline wrapper for HF models
async def async_pipeline_infer(pipeline, text: str) -> List[Dict[str, Any]]:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, lambda: pipeline(text))
...
class ZenarkResponse(BaseModel):
    validation: str = Field(description="Short validation of user's feeling, 10-15 words.")
    reflection: str = Field(description="Concise reflective sentence showing understanding.")
    question: str = Field(description="Open-ended question to encourage elaboration.")
...
class AsyncMongoChatMemory:
    def __init__(self, session_id: str, chats_col: AsyncIOMotorCollection):
        ...
        Args:
            ...
            chats_col: MongoDB collection for storing chat messages.
        """
    ...
    async def _load_existing(self) -> None:
        """
        Load existing chat history from MongoDB asynchronously.
        """
        try:
            ...
            logger.warning(f"Failed to load chat history for session {self.session_id}: {e}")

    async def append_user(self, text: str) -> None:
        """
        Append user message to history and MongoDB.
        """
        ...
        try:
            ...
            logger.warning(f"Failed to save user message for session {self.session_id}: {e}")

    async def append_ai(self, text: str) -> None:
        """
        Append AI message to history and MongoDB.
        """
        ...
        try:
            ...
            logger.warning(f"Failed to save AI message for session {self.session_id}: {e}")
...
async def get_academic_marks(student_name: str, marks_col: AsyncIOMotorCollection) -> str:
    """
    Retrieve academic marks from MongoDB for the given student asynchronously.
    
    Args:
        student_name: Name of the student to query.
        marks_col: Async MongoDB collection for marks.
    
    Returns:
        Formatted string of academic marks or error message.
    """
    try:
        record: Optional[Dict[str, Any]] = await marks_col.find_one(
        ...
        logger.error(f"Error fetching marks for {student_name}: {e}")
        return f"Error retrieving marks for {student_name}."
...
async def fetch_marks_direct(student_name: str, marks_col: AsyncIOMotorCollection) -> str:
    """
    Direct marks fetch (bypass agent) for async MongoDB query.
    ...
    Returns:
        Formatted string of marks or "null" if not found.
    """
    try:
        record: Optional[Dict[str, Any]] = await marks_col.find_one(
            {"name": {"$regex": f"^{student_name}$", "$options": "i"}}
        ...
        logger.error(f"Error in fetch_marks_direct for {student_name}: {e}")
        return "null"
...
# Revised fetch_marks_node
async def fetch_marks_node(state: GraphState, config: Optional[RunnableConfig] = None) -> Dict[str, Any]:
    """
    Fetch academic marks for the user asynchronously.

    Args:
        state: GraphState containing user information.
        config: Optional RunnableConfig containing marks_col.

    Returns:
        Dict with 'marks' key containing formatted marks or fallback message.
    """
    # Get marks_col from config or use global marks_col
    if config and isinstance(config, dict):
        marks_col = config.get("marks_col")
    else:
        marks_col = None
    
    # Fallback to global marks_col if not in config
    if not marks_col:
        marks_col = globals().get("marks_col")
    
    if not marks_col:
        logger.error("marks_col not provided in config or global scope")
        return {"marks": "didn't got marks"}
    ...
    try:
        ...
        logger.warning(f"Marks fetch failed for {name}: {e}. Returning fallback message.")
        marks = "didn't got marks"
    return {"marks": marks}
...
# NEW: Concurrent node that executes score_emotion, fetch_marks, and retrieve_rag in parallel
async def parallel_gather_node(state: GraphState) -> Dict[str, Any]:
    """
    Execute score_emotion, fetch_marks, and retrieve_rag concurrently.
    This reduces latency from O(n) sequential to O(1) parallel for these three operations.
    """
    logger.info("Executing score_emotion, fetch_marks, retrieve_rag in PARALLEL...")
    
    # Create async tasks for each node (nodes are sync, so we use asyncio.to_thread)
    async def run_score_emotion():
        try:
            return await asyncio.to_thread(score_emotion_node, state) or {}
        except Exception as e:
            logger.warning(f"score_emotion failed: {e}")
            return {}
    
    async def run_fetch_marks():
        try:
            return await asyncio.to_thread(fetch_marks_node, state) or {}
        except Exception as e:
            logger.warning(f"fetch_marks failed: {e}")
            return {}
    
    async def run_retrieve_rag():
        try:
            return await asyncio.to_thread(retrieve_rag_node, state) or {}
        except Exception as e:
            logger.warning(f"retrieve_rag failed: {e}")
            return {}
    ...
    logger.info(f"Parallel gather completed in {elapsed:.2f}s (would be ~3x slower sequentially)")
...
# Now route from classify_category directly to parallel_gather (skips intermediate nodes)
workflow.add_edge("classify_category", "parallel_gather")
workflow.add_edge("parallel_gather", "get_guideline")
...
logger.info("LangGraph workflow compiled successfully with PARALLEL gather node.")
...
async def get_session_history(session_id: str) -> ChatMessageHistory:
    """
    Fetch or initialize persistent chat history for this session.
    Loads from MongoDB on demand and syncs every message asynchronously.
    
    Args:
        session_id: Unique identifier for the chat session.
    
    Returns:
        ChatMessageHistory object with session history.
    """
    if chats_col is None:
        raise ValueError("MongoDB collection not initialized. Call init_db first.")
    memory = AsyncMongoChatMemory(session_id, chats_col)
    return memory.get_history()  # Synchronous call, no await needed

# ============================================================
#  TOKEN COUNTING HELPERS
# ============================================================
try:
    import tiktoken  # Optional: best accuracy for OpenAI-style token counts
except Exception:
    tiktoken = None

def _get_token_encoder(model_name: str = "gpt-4o-mini"):
    """
    Try to get a tokenizer/encoder for the model. If tiktoken is not installed
    we fall back to a very simple heuristic (chars/4).
    """
    if tiktoken:
        try:
            # prefer model-specific encoder; fall back to cl100k_base
...
def count_tokens(text: str, model_name: str = "gpt-4o-mini") -> int:
    """
    Return an estimated token count for a single string.
    Uses tiktoken when available, otherwise estimates as len(chars)/4.
    (This is an approximation and intended for monitoring/cost-estimates.)
    """
    if not text:
        return 0
...
def count_tokens_for_history(history_obj, include_last_n: int = 6, model_name: str = "gpt-4o-mini") -> int:
    """
    Count tokens for the recent conversation context.
    history_obj is the ChatMessageHistory or similar; we walk its messages
    and sum token counts for the last `include_last_n` turns.
    """
    try:
        msgs = list(history_obj.messages)[-include_last_n:]
    except Exception:
        # if history_obj isn't as expected, treat as empty
        msgs = []
    total = 0
    for m in msgs:
        try:
            ...
            content = str(m)
        total += count_tokens(str(content), model_name=model_name)
    return total
...
async def chat(request: Request):
    """
    Robust async /chat endpoint with caching for common queries.
    Accepts flexible JSON shapes: {"message":...}, {"text":...}, {"input":...}, or Pydantic ChatRequest shape.
    Returns JSON with response and token accounting (context, user, input, response, total).
    
    NEW: Implements response caching with TTL to avoid redundant LLM calls.
    For similar user inputs + sentiment + category, returns cached response (10min TTL).
    """
    try:
    ...
    session_id = data.get("session_id") or data.get("session") or "anonymous"

    # Flexible user text extraction (support multiple client shapes)
    user_text = (
        data.get("message") or data.get("text") or data.get("input") or data.get("user_text")
    )

    parsed_req = None
    if not user_text:
        # Try to validate against ChatRequest if client used that shape
        try:
            parsed_req = ChatRequest(**data)
            user_text = parsed_req.text
            session_id = parsed_req.session_id or session_id
        except Exception:
            return JSONResponse(status_code=400, content={"error": "Missing required 'message' or 'text' field in request body."})
    ...
    # Extract additional params with sensible fallbacks
    name = data.get("name") or (parsed_req.name if parsed_req is not None else None)
    question_index = data.get("question_index") or (parsed_req.question_index if parsed_req is not None else 1)
    max_questions = data.get("max_questions") or (parsed_req.max_questions if parsed_req is not None else 5)
    
    # NEW: Quick sentiment detection for cache key (simplified detection without full graph)
    sentiment = "neutral"
    try:
        from prompt import detect_end_intent
        if not detect_end_intent(user_text):
            # Simple heuristic: check for positive/negative keywords
            positive_keywords = ["good", "great", "happy", "wonderful", "love", "amazing", "excited", "proud"]
            negative_keywords = ["sad", "angry", "frustrated", "worried", "scared", "depressed", "anxious", "stress"]
            user_lower = user_text.lower()
            pos_count = sum(1 for kw in positive_keywords if kw in user_lower)
            neg_count = sum(1 for kw in negative_keywords if kw in user_lower)
            sentiment = "positive" if pos_count > neg_count else ("negative" if neg_count > 0 else "neutral")
    except Exception:
        sentiment = "neutral"
    
    # Try cache hit BEFORE generating response
    cached_response = response_cache.get(user_text, "general", sentiment)
    if cached_response:
        logger.info(f"Returning CACHED response for: {user_text[:50]} | sentiment={sentiment}")
        ...
        response_tokens = count_tokens(cached_response)
        return JSONResponse(content={
            ...
            "tokens": {
                ...
                "total": user_tokens + response_tokens,
                "note": "This response was retrieved from cache (faster path)"
            }
        })
    ...
    # Generate the AI response using async pipeline
    ai_result = await generate_response(user_text, name=name, question_index=question_index, max_questions=max_questions, session_id=session_id, marks_col=marks_col)

    # ai_result may be a dict {"response":..., "prompt_tokens":...} or a plain string (fallback)
    if isinstance(ai_result, dict):
        ai_text = ai_result.get("response", "")
        prompt_input_tokens = int(ai_result.get("prompt_tokens", 0) or 0)
    else:
        ai_text = str(ai_result)
        prompt_input_tokens = 0

    # NEW: Store in cache for future similar queries
    response_cache.set(user_text, "general", sentiment, ai_text)

    # Store the AI message immediately
    await memory.append_ai(ai_text)

    # --- Token accounting (simple, transparent) ---
    history = memory.get_history()
    context_tokens = count_tokens_for_history(history, include_last_n=6)
    user_tokens = count_tokens(user_text)
    response_tokens = count_tokens(ai_text)
    total_tokens = context_tokens + user_tokens + response_tokens + prompt_input_tokens

    logger.info(
        f"Token usage | session={session_id} | context={context_tokens} | user={user_tokens} "
        f"| input={prompt_input_tokens} | response={response_tokens} | total={total_tokens}"
    )

    # Return response + token metadata for monitoring/cost estimation
    return JSONResponse(content={
        "response": ai_text,
        "from_cache": False,
        "tokens": {
            ...
            "response": response_tokens,
            "total": total_tokens,
            "note": "If you need exact billing tokens for your API provider, install and use the provider tokenizer (e.g., tiktoken) and ensure model name matches the encoder."
        }
    })
